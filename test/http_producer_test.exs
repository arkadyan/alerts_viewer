defmodule HttpProducerTest do
  ...
  describe "handle_info/2" do
    ...
    test "ignores unknown messages" do
      ...
      state = %HttpProducer.State{machine: machine}
    ...
    test "does not send more demand than requested" do
      machine = StateMachine.init("url", parser: &[&1])
      state = %HttpProducer.State{machine: machine, demand: 1}
  ...
  describe "bypass" do
    ...
    test "a fetch error is not fatal" do
      {:ok, pid} = start_supervised({HttpProducer, {"nodomain.dne", parser: & &1}})
    ...
    defp start_producer(bypass, opts \\ []) do
      ...
      {:ok, _} = start_supervised({HttpProducer, {url, opts}})
    end
