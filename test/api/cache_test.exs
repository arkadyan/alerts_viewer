defmodule Api.CacheTest do
  ...
  describe "expire!/0" do
    test "removes the least-recently-used items to get the cache down to the right size", %{
      ...
      _ = Cache.cache_response(name, "one", @params, @response)
      _ = Cache.cache_response(name, "expire", @params, @response)
      _ = Cache.cache_response(name, "also expire", @params, @response)
      _ = Cache.cache_response(name, "one", @params, %{@response | status_code: 304})
